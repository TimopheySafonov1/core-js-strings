...
function isPalindrome(str) {
  ...
  for (let i = cleanedStr.length - 1; i >= 0; i -= 1) {
    temp += cleanedStr[i].toLowerCase();
  }
  return temp === cleanedStr;
}
...
function findLongestWord(sentence) {
  const letStr = sentence.replace(/[^a-zA-Z0-9]/g, ' ');
  const results = letStr.split(' ');
  let logestWorld = '';
  let max = 0;
...
function reverseWords(str) {
  const splittedWorld = str.split(' ');
  let reversedString = '';
  for (let i = 0; i < splittedWorld.length; i += 1) {
    const reverseWord = `${splittedWorld[i].split('').reverse().join('')} `;
...
function extractNameFromTemplate(value) {
  ...
  const lastWithout = seperatedValue[2].slice(0, -1);
  return `${seperatedValue[1]} ${lastWithout}`;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    const rot13 = (index + 13) % 26;
    return isUpper ? alphabet[rot13] : alphabet[rot13].toLowerCase();
  });
}
